...
def create_table():
    '''this function opens up a connection to a database called habits.db and creates the table habits if it doesn't exist yet'''
...
def dummy_data():
    ...
    #insert dummy data if table habits empty
    if len(results) == 0:
...
def habit_broken():
    ...
    connection = sqlite3.connect('habits.db')
    cursor = connection.cursor()
    #comparing check_off_next with todays date to discover any broken streaks
